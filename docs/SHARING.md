...
| 30 | 沉淀只能「原位替换」+ 路由读不懂语义，知识无法跨流程复用 | 架构 | 高（能力上限） |
| 31 | 沉淀产物在控制台里看不见也删不掉（旧列表只认有版本链的流程） | Web 控制台 | 中（资产失控） |
...
### 30：沉淀只能「原位替换」+ 路由读不懂语义——知识无法跨流程复用

**现象**：老板追问"为什么沉淀要同时满足四个条件？我认为应该是成功了就蒸馏，下次用大模型判断是否命中，命中就走沉淀"。他的直觉是对的，这一问暴露出两个架构缺口。

**根因**（不是 bug，是架构缺失）：

1. **没有独立轨迹库** → 蒸馏产物必须有落点，只能焊回原 Playbook 的 `failedIndex`；也只能用 `resumedIndex` 证明"这次成功是自愈"而非"跑偏了但看起来成功"。所谓"四个条件"里，真正多余的正是这一对——它们是缺轨迹库时的**替代品**。
2. **没有意图层** → `selectPlaybook` 只选流程、不抽参数，且单候选时跳过 LLM。例：Playbook 明明支持 `loop over: ${params.items}`（改 1 个还是 3 个由数组长度决定），却**没有任何环节把"把 S001/S002/S003 都改成 188"变成 items 数组**——只能手填参数框。

**修复（四层）**：

| 层 | 文件 | 做法 |
|---|---|---|
| F-10 独立蒸馏 | `src/learner/draft.ts` | Agent 成功 → 创建**全新** Playbook（自带 baseUrl/allowDomains/起始 goto），不再依赖已有 Playbook |
| 意图解析层 | `src/router/intent.ts` | 每次 LLM 解析 → `{playbook, params, confidence}`，一次调用同时完成选流程 + 抽参数 + 自评置信度 |
| 可配档位 | `server/index.ts` + Web | `learnMode`: off / on-failure / on-success；`routeMode`: deterministic / intent |
| 安全补丁 | `draft.ts` 末尾 assert | 沉淀时记录终态 URL path，下次执行不符 → assert 失败 → 自动回退 Agent |

**通用教训**（四条，第 4 条最容易被自己坑）：

1. **知识是「编译进流程」还是「运行时检索」** —— 前者零成本但只服务单个 Playbook 的单步骤；后者跨场景复用但要付检索费。不是二选一，是规模问题。
2. **省 token 靠缓存，不靠跳过解析** —— 跳过解析 = 读不懂语义 = 回到原始问题。正确姿势是意图结果缓存（key = 归一化任务 + 域名），高频重复任务命中率接近 100%，既保语义又拿 ¥0。
3. **fail-safe 变 fail-dangerous 必须补断言** —— 旧架构"先跑 Playbook，失败才 Agent"，误判顶多浪费钱；新架构"先判断再执行"，误判会**静默执行错流程**。所以必须双保险：置信度阈值（<0.7 走 Agent）+ 执行后成功断言。
4. **伪折中常源于维度混淆** —— 我最初提的 `hybrid`（候选 ≤1 且无自由文本 → 直选）被一眼看穿：**候选数量只影响解析成本，有无自由文本才决定是否需要语义理解**。把两个无关维度揉进一个条件，就是伪折中。修正后砍掉 hybrid，只留两档，且**默认值跟着入口走**（Web 自然语言入口 = intent，CLI/显式指定 = deterministic），用户不需要理解"策略"这个概念。

**验证**（mock 轨迹 + 真实 LLM）：

```
6 步轨迹 → 沉淀：跳过密码框 → 5 步业务步骤 + 首步 goto + 末尾 assert(/sku/list)
           参数化：填 188 → ${params.price}（语义化命名，不是 value1）
意图解析：同任务        命中 0.92，params={price:'188'}
         再跑一次      命中缓存（零调用）
         "把商品全删除" 未命中，置信度 0.05（正确拒识）
```

---

### 31：沉淀产物在控制台里"看不见、删不掉"（W13，Web 控制台）

**现象**：W12 做完 F-10 自动沉淀后，Agent 每次成功都会往 `playbooks/` 写一条新流程，但 Web 控制台里**看不到它们**——主页「已沉淀流程」和会话里「⚡ 沉淀流程」两个入口都调 `/api/playbooks`，而那个接口的实现是「扫 `.versions/` 目录，没有版本链的跳过」：

```ts
if (!existsSync(dir)) continue; // 从未沉淀过的 Playbook 不展示
```

于是形成死循环：**自动沉淀出来的全新草稿没有版本链 → 列表不显示 → 用户不知道它存在 → 没法复核也没法删 → 只会越攒越多**（沉淀错了的流程还会被意图层命中，等于埋雷）。

**根因**：把「沉淀资产」和「版本历史」两套语义塞进了一个接口。前者问的是"现在有哪些流程"，后者问的是"这个流程改过几版"——F-10 之前两者恰好等价（有版本链 = 有流程），F-10 之后不等价了。

**修复**：新增沉淀库（`/api/drafts`），与版本历史接口职责分离：

| 接口 | 语义 | 覆盖范围 |
|---|---|---|
| `GET /api/playbooks` | 版本链（谁改过、当前 v 几） | 仅 `.versions/` 存在的流程 |
| `GET /api/drafts` | 沉淀资产清单 | `playbooks/` 下**全部** `.yaml/.yml` |
| `GET /api/drafts/:name` | 单条明细（步骤/参数/站点/源码/版本链） | — |
| `DELETE /api/drafts/:name` | 删除（默认连 `.versions/` 一起删，`?versions=0` 可保留历史） | — |

配套前端 `web/src/components/DraftsView.vue`：左列表 + 右明细（目标站点 / 允许域名 / 参数 / 来源 / 校验状态 / 版本链 / 完整步骤 / YAML 源码），删除走二次确认；侧边栏加「⚡ 沉淀库」入口，主页「已沉淀流程」改为读新接口并附「沉淀库 →」跳转。

**三个实现取舍**：

1. **坏流程也要能看、能删**——`readOne()` 里 `loadPlaybook` 校验失败时不抛错，降级为直接 `yaml.load` 读原始步骤，把错误信息显示在「校验」字段。否则一条坏 YAML 会把整个列表接口打成 500，用户连删的入口都没了。
2. **删除必须双重校验路径**——名称先过白名单 `/^[A-Za-z0-9._-]+$/`，再校验 `resolve(file)` 落在 `playbooks/` 内。用户可控字符串进 `unlinkSync` 是高风险操作。
3. **删完要 `clearIntentCache()`**——意图缓存里还留着已删流程的命中记录，不清会导致删完还能被"命中"，执行时才 404。

**教训**：**新增一种"产出物"时，要回头检查所有"资产列表"的过滤条件是否还成立**。旧过滤条件（有版本链才展示）在旧语境下是对的，新产物一出现就变成了隐性屏蔽——而且不会报错，只是静默少显示。这类 bug 比崩溃难发现得多。
